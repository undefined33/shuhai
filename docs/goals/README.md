...
> 最后更新：2026-07-14
> 当前状态：Goal 041/042、Goal 043A fixture-only 候选与 043B v2 合同已通过；Goal 043 仍等待真实 X 人工门禁
> 执行规则：当前为 `CONTRACT_PASS_WAITING_MANUAL_GATE`；用户完成隔离测试账号门禁前不得实施生产接线或访问真实 X。
...
| Goal 043 | `BLOCKED_BY_REAL_X_EVIDENCE` | 043A 与 043B v2 合同均 `PASS`            | 人工隔离登录 + 10 条 probe 门禁 |
| 044-046  | `PLANNED`                    | 微博、书签和 UI                          | 按各自前置另写可执行 spec       |
| Goal 047 | `RESEARCH_GATE`              | 是否支持知乎、小红书或其它平台           | 两周 dogfood 证明真实需求       |
| workflow | `043B_WAITING_MANUAL_GATE`   | 合同已 PASS；生产实现尚未 READY          | 用户完成人工隔离账号门禁        |
...
| [043](./goal-043-x-bookmarks-incremental-sync.md)      | `BLOCKED_BY_REAL_X_EVIDENCE` | X 收藏增量同步 MVP                        | 043A/合同 PASS；待人工登录门禁   |
