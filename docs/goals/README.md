...
> 最后更新：2026-07-14
> 当前状态：Goal 041/042、Goal 043A fixture-only 候选与 043B v2 合同已通过；Goal 043B 离线生产实现进行中
> 执行规则：当前为 `IN_PROGRESS_OFFLINE_IMPLEMENTATION`；只允许合同内生产接线、测试和离线 E2E。真实 X probe 等 Codex Chrome 连接恢复后再执行，仍只允许用户指定的单个 X 收藏页标签。
...
| Goal 043 | `IN_PROGRESS`                 | 043B 离线生产实现；真实 QA 后置          | 合同与用户边界授权已满足        |
| 044-046  | `PLANNED`                     | 微博、书签和 UI                          | 按各自前置另写可执行 spec       |
| Goal 047 | `RESEARCH_GATE`               | 是否支持知乎、小红书或其它平台           | 两周 dogfood 证明真实需求       |
| workflow | `043B_OFFLINE_IMPLEMENTATION` | 生产接线、测试、fixture E2E              | 实现门禁后等待真实 Chrome probe |
...
| [043](./goal-043-x-bookmarks-incremental-sync.md)      | `IN_PROGRESS`             | X 收藏增量同步 MVP                        | 043B 离线生产实现                |
