...
建议把主壳切换设为前三个 v4 模块完成后的下一个实施模块，先于 Goal 045 的书签流程扩建：
...
第一版可见切换应在 046A 的首个实现 PR 完成，而不是等全部书签功能重写后再统一换皮。046A 只改变入口、布局和加载边界，保留现有领域逻辑与持久化合同。
...
1. 046A 作为 Goal 043 之后的下一实施模块，先于 Goal 045。
