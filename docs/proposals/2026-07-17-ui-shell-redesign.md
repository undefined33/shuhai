...
> 日期：2026-07-17
>
> 状态：`IMPLEMENTED THROUGH GOAL 046B / GOAL 046C QA PENDING`
>
...
Goal 046A/046B 已把生产入口切换到 v4：Popup 是上下文单动作，Side Panel 只承载当前
书签或 X 任务，Options 是独立入口；旧版通用 `App.tsx` 与失去入口的后台页面已从生产
依赖图删除。尚未完成的是 Goal 046C 的完全隔离浏览器视觉、可访问性和实际挂载验收，
不是继续扩张功能。
...
Goal 043 DONE
  ...
  -> 045B/045C P1 信任边界收口
  -> 046A 主壳、视觉系统与按需加载
  -> 046B Options 与跨任务一致性
  -> 046C 隔离 E2E；两周 dogfood 是后续人工门禁
```
...
- 分类建议属于“整理书签”。重复检测尚无可验收算法，等待独立 Goal；失效链接只保留旧
  记录的人工核实入口，不恢复主动扫描。
- 扫描、复核、去重、写入属于“同步收藏”。
...
- 两周 dogfood 不属于 046B 自动化完成条件；当前拆分与状态以 Goal 046B/046C 和 v4
  路线图为准。
...
本提案已分别转换为 Goal 046A/046B 并完成独立合同、实现 review 和完整门禁。剩余视觉、
可访问性、主题、缩放、焦点与实际挂载证据只能由 Goal 046C 的隔离浏览器合同验收；
两周真实 dogfood 仍是后续人工门禁。
